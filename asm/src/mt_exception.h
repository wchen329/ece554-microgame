...
namespace asmrunner
{
	...
	{
		public:
			mt_no_such_cpu_option(const std::string& option_name)
			{
				mt_exception::except_num = exception_nums::NO_SUCH_CPU_OPTION;
				mt_exception::message = (std::string("The option ") + std::string(option_name) + std::string(" does not exist for this CPU."));
			}
	};
	...
	{
		public:
			mt_io_file_open_failure(const std::string& filename)
			{
				mt_exception::except_num = exception_nums::IO_FILE_OPEN_FAILURE;
				mt_exception::message = (std::string("Failed to open file ") + std::string(filename) + std::string("."));
			}
	};
	...
	class mt_io_device_type_mismatch : public mt_exception
	{
		public:
			mt_io_device_type_mismatch()
			{
				...
				mt_exception::message = std::string("A request was sent to a device, but the device does not understand that particular device. The request as a result has been ignored.");
			}
	...
	class mt_io_hdd_rw_io_failure : public mt_exception
	{
		public:
			mt_io_hdd_rw_io_failure()
			{
				...
				mt_exception::message = std::string("A read or write to an HDD failed.");
			}
	...
	class mt_io_hdd_seek_failure : public mt_exception
	{
		public:
			mt_io_hdd_seek_failure()
			{
				...
				mt_exception::message = std::string("Seeking to the specified address for HDD I/O failed.");
			}
	};
}
